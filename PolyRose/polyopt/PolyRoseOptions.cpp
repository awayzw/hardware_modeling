...
PolyRoseOptions::parse(int argc, char** argv)
{
  for (int i = 1; i < argc; ++i)
      ...
      if (! strcmp (argv[i], "--polyopt-letsee-space"))
	{
      ...
      check_opt_val(argv[i], "--systemc-target-func",
    	systemc_target_func);
      check_opt_val(argv[i], "--systemc-testbench-func",
    	systemc_testbench_func);
      check_opt(argv[i], "--systemc-no-pretile",
    	systemc_pretile=false);
      check_opt_val(argv[i], "--systemc-depth",
    		  systemc_depth);
      check_opt_val(argv[i], "--systemc-preprocess",
    		  systemc_preprocess);
      //std::cout << "\n\n\nparsing preprocess is " << systemc_preprocess << std::endl;
      /*check_opt_val(argv[i], "--systemc-unroll-factor-file",
      	systemc_unroll_factor_file);*/
