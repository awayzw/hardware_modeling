...
SC_FunctionModule::SC_FunctionModule(SgFunctionDeclaration* func, SgScopeStatement* scope, int depth, bool isroot)
: modulefunc(func)
 ,parentscope(scope)
 ,moduleclass(0)
 ,latencyVar("")
...
void SC_FunctionModule::process(std::vector<string> globalUnrollFactor, bool no_pretile)
{
	...
	if(depth_below_me > 0)
	{
		...
		for(int i=0; i<topForLoops.size(); ++i)//vector<SgForStatement*>::iterator it=topForLoops.begin(); it != topForLoops.end(); ++it)
		{
			...
			SC_FunctionModule* childModule = new SC_FunctionModule(moduleFunc, this->parentscope, depth_below_me-1, false);
			stringstream sslat;
			...
			replaceText << endl << "// send signal for loop" << i << "modules to start\n"
						<< "for(int loopi=0; loopi<" << iterations <<" ; ++loopi)\n"
						<< "{\n";
			for(int j=0; j<childModule->member_names.size(); j++)
			{ 
				if(childModule->member_names[j]==iterator)
				{
					replaceText << tab <<  gen_prefix << "tiles" << i << "_" << childModule->member_names[j] << "[loopi]=(" << it_start << "+(loopi*" << it_inc << "));\n";
				}
...
void SC_FunctionModule::addToFile()
{
	...
	SageInterface::appendStatement(moduleclass, parentscope);
...
int TransformToSystemC(SgProject* project, string targetname, string testbenchname, int depth, std::vector<string> unroll_factors, bool preprocess, bool no_pretile)
{
	...
	SC_FunctionModule* rootModule = new SC_FunctionModule(targetFunction, file->get_globalScope(), depth, true);
